...
- **Model**: {モデル名}を新規作成・更新
- **Controller**: {コントローラー名}でアクションを実装
- **View**: {ビュー名}の UI を作成・更新
- **Routes**: ルーティングを追加・変更
- **Migration**: データベーススキーマを更新
- **Test**: RSpec テストを追加・更新

### 技術仕様

- **ActiveRecord**: {関連・バリデーション}を定義
- **認証・認可**: {Devise/CanCan等}の設定
- **API**: RESTful エンドポイントを{追加/更新}
- **フロントエンド**: {Turbo/Stimulus/ViewComponent}を使用

### データベース変更

- **テーブル**: `{テーブル名}`を{作成/更新}
- **カラム**: `{カラム名}`を追加（型: {データ型}）
- **インデックス**: パフォーマンス向上のため追加
- **外部キー**: `{関連テーブル}`との関連を設定

### セキュリティ

- **Strong Parameters**: 許可パラメータを適切に設定
- **認証**: ログイン必須ページの制御
- **認可**: ロールベースアクセス制御
- **CSRF**: トークン検証を実装
```
